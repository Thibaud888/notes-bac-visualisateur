...
const DEFAULTS = {
  ...
  labels: {},
  tab: "pie",          // représentation : pie | treemap | radar
...
function migrate(o) {
  ...
  if (o.tab === "objectif" || o.tab === "bars") o.tab = "pie";
  if (!["pie", "treemap", "radar"].includes(o.tab)) o.tab = "pie";
  if (!["matiere", "domaine", "type"].includes(o.colorBy)) o.colorBy = "matiere";
  o.focusLocked = !!o.focusLocked;
  o.target = Number(o.target) || 10;
  return o;
}
...
function renderTreemap(m) {
  ...
  let svg = `<svg viewBox="0 0 ${W} ${H}" class="treemap" role="img" aria-label="Treemap par matière">`;
...
function renderRadar(m) {
  ...
  let svg = `<svg viewBox="0 0 420 300" class="radar" role="img" aria-label="Radar des moyennes par domaine">`;
